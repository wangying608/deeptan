...
function genWindowsAndSlides(num_sample::Int64,
    ...
    @fastmath sum_move = sum(num_move)
...
function slidingWindow2D(Algor::Function, getOnlyOptimum::Bool,
    ...
    for xm in eachindex(col_results)
...
function rmOutliersVecXY(X::Union{AbstractVector{Float64}, AbstractVector{Int64}}, Y::Union{AbstractVector{Float64}, AbstractVector{Int64}},
                         detectX::Vector{Int64} = detectOutliers(X),
                         detectY::Vector{Int64} = detectOutliers(Y),
    ...
    elem2rm::Vector{Int64} = unique(vcat(detectX, detectY))
    # sort!(unique!(elem2rm))
    elem2save::Vector{Int64} = collect(Int64, 1:lenX)[InvertedIndex(elem2rm)]
    sort!(elem2save)
    if length(elem2rm) > 0
        if getOnlyPosition
            return elem2save
...
function binsFD(v::Union{AbstractVector{Float64},AbstractVector{Float32},AbstractVector{Int64}})::Int64
    ...
    iqr_v = q3 - q1
    @fastmath bin_width = 2 * iqr_v * vLen ^ (-1/3)
...
function histogramX1(arrayin::Union{AbstractArray{Int},AbstractArray{Float64},AbstractArray{Float32}},
    ...
    bin_width = @fastmath (maxX - minX) / nbins
...
function myMIXY(x::Union{AbstractVector{Int},AbstractVector{Float64},AbstractVector{Float32}},
    ...
    pXY = @fastmath pXY ./ length(x)
    pX = @fastmath sum(pXY, dims=2)
    pY = @fastmath sum(pXY, dims=1)
    #
    oMI = 0.0
    for i in 1:(nBinsX * nBinsY)
        xi = @fastmath div(i-1, nBinsY) + 1
        yi = @fastmath mod(i-1, nBinsY) + 1
        if pXY[xi,yi] > 0.0
            @fastmath oMI += pXY[xi,yi] * log2(pXY[xi,yi] / (pX[xi] * pY[yi]))
        end
    end
    #
...
### matrix modifier
function atomMI(pair_name::String,
                f1_vec::Union{AbstractVector{Float64}, AbstractVector{Int64}}, f2_vec::Union{AbstractVector{Float64}, AbstractVector{Int64}},
    ...
    outs = Dict(pair_name => Dict("mi_max" => optimumxs.MI_max, "samples_used" => wh2save, "feature_1" => xro, "feature_2" => xco))
    return outs
end
...
function pairNames(aCombn::Vector{Int64})
    return string("pair_", aCombn[1], "-", aCombn[2])
end
...
function atomsMIs(X::Union{AbstractMatrix{Float64}, AbstractDataFrame},
                  ...
                  slideStep::Union{Int64, Float64}, widthRange::Vector{Float64}, widthStep::Float64)
    tAtom(aCombn::Vector{Int64}) = atomMI(pairNames(aCombn), view(X,:,aCombn[1]), view(X,:,aCombn[2]), outlierXs[aCombn[1]], outlierXs[aCombn[2]], slideStep, widthRange, widthStep)
    resultMIandSample = tmapreduce(tAtom, merge, combinations(1:size(X,2), 2))
    return resultMIandSample
end
function atomsMIs(X::Union{AbstractMatrix{Float64}, AbstractDataFrame},
                  slideStep::Union{Int64, Float64}, widthRange::Vector{Float64}, widthStep::Float64)
    tAtom(aCombn::Vector{Int64}) = atomMI(pairNames(aCombn), view(X,:,aCombn[1]), view(X,:,aCombn[2]), slideStep, widthRange, widthStep)
    resultMIandSample = tmapreduce(tAtom, merge, combinations(1:size(X,2), 2))
    return resultMIandSample
end
### X-Y
function atomsMIs(X::Union{AbstractMatrix{Float64}, AbstractDataFrame},
    ...
    tAtom(aCombn::Vector{Int64}) = atomMI(pairNames(aCombn), view(X,:,aCombn[1]), view(Y,:,aCombn[2]), outlierXs[aCombn[1]], outlierYs[aCombn[2]], slideStep, widthRange, widthStep)
    resultMIandSample = tmapreduce(tAtom, merge, combns)
    return resultMIandSample
end
function atomsMIs(X::Union{AbstractMatrix{Float64}, AbstractDataFrame},
                  Y::Union{AbstractMatrix{Float64}, AbstractDataFrame},
    ...
    #
    combns = [[x,y] for x in 1:size(X, 2) for y in 1:size(Y, 2)]
    tAtom(aCombn::Vector{Int64}) = atomMI(pairNames(aCombn), view(X,:,aCombn[1]), view(Y,:,aCombn[2]), slideStep, widthRange, widthStep)
    resultMIandSample = tmapreduce(tAtom, merge, combns)
    return resultMIandSample
end
