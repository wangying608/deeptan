...
class AMSGP(torch.nn.Module):
    ...
    def forward(self, node_names, x, edge_attr, edge_index, batch):
        ...
        for graph_id in unique_batches:
            ...
            local_node_ids = torch.arange(mask.sum(), device=self._device)
            global_to_local = torch.zeros_like(mask, dtype=torch.long, device=self._device)
    ...
    def _generate_multiscale_subgraphs(self, edge_index, centrality, h) -> List[GData]:
        ...
        for mask, center in zip(subgraph_masks, subgraph_centers):
            ...
            subgraphs.append(
                GData(
                    x=h[node_idx],
                    edge_index=self._fast_edge_index(edge_index, mask, node_idx),
                    center_node=center,
                    node_idx=node_idx,
                    mask=mask,
                )
    ...
    @staticmethod
    @torch.jit.script
    def _fast_edge_index(edge_index: torch.Tensor, mask: torch.Tensor, node_idx: torch.Tensor) -> torch.Tensor:
        # Vectorized edge index filtering
        edge_mask = mask[edge_index[0]] & mask[edge_index[1]]
